<template>
  <div class="app-container">
    ...
    >
      ...
      <!-- 一列的结束 -->
      <el-table-column label="券码类型名称" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.couponCodeType }}</span>
        </template>
      </el-table-column>
      <el-table-column label="类型描述" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.typeDescribe }}</span>
        </template>
      </el-table-column>
        <el-table-column prop="image" label="商铺类型图片" align="center" min-width>
        <!-- 图片的显示 -->
...
<script>
import { findTypePage } from "@/api/shopsManage";
...
export default {
  ...
  methods: {
    ...
    async findAll(data) {
      ...
      const res = await findTypePage(this.listQuery);

      const item = res.shopsTypeList.list;
      this.list = [];
      // 遍历返回的list
      item.map(value => {
        const temp = value;

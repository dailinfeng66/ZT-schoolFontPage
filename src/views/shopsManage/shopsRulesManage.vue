<template>
  <div class="app-container">
    ...
    >
      ...
      <!-- 一列的结束 -->
      <el-table-column label="商铺名称" width align="center">
        <template slot-scope="{ row }">
      ...
      <el-table-column label="积分使用限制" width="200px" align="center">
        <template slot-scope="{ row }">
          <span>{{ row.integralLimit }}</span>
        </template>
      </el-table-column>
    
      <el-table-column label="商铺手续费 " width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.poundage }}</span>
        </template>
      </el-table-column>
...
<script>
import { findRulesPage,findShopById } from "@/api/shopsManage";
...
export default {
  ...
  methods: {
    ...
    async findAll(data) {
      ...
      const res = await findRulesPage(this.listQuery);

      const item = res.shopsrulesList.list;
      this.list = [];
      // 遍历返回的list
      item.map(value => {
        const temp = value;
         this.findShopById(temp.shopsId).then(name => {

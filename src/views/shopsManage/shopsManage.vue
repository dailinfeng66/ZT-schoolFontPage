<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="商铺类型" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.type }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="image" label="商铺图片" align="center" min-width>
        <!-- 图片的显示 -->
      ...
      <el-table-column label="商铺描述" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.shopsDescribe }}</span>
        </template>
      </el-table-column>

      <el-table-column label="营业时间" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.businessTime }}</span>
        </template>
      </el-table-column>
      <el-table-column label="商铺申请id" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.applyId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="是否删除 " width align="center">
        <template slot-scope="{ row }">
...
<script>
import { findShopPage,findTypeById } from "@/api/shopsManage";
...
export default {
  ...
  methods: {
    ...
    async findAll(data) {
      ...
      const res = await findShopPage(this.listQuery);

      const item = res.shopsList.list;
      this.list = [];
      // 遍历返回的list
      item.map(value => {
        const temp = value;

        this.findTypeById(temp.type).then(name => {
           
             temp.type = name;
           });
        if (temp.isDelete == 1) {
          temp.isDelete = "未删除";
        } else if (temp.isDelete == 0) {
          temp.isDelete = "删除";
        }
        this.list.push(temp);
      });

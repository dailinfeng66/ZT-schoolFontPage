<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="居民id" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.residentId }}</span>
        </template>
      </el-table-column>

      <el-table-column label="商铺码" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.shopsCode }}</span>
        </template>
      </el-table-column>
      <el-table-column label="商铺类型id" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.shopsTypeId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="商铺名字" width="200px" align="center">
        <template slot-scope="{ row }">
          <span>{{ row.name }}</span>
        </template>
      </el-table-column>
       <el-table-column prop="image" label="商铺图片" min-width>
        <!-- 图片的显示 -->
        <template slot-scope="scope">
          <img :src="scope.row.image" min-width="70" height="70" />
        </template>
      </el-table-column>
      <el-table-column label="商铺地址" width="200px" align="center">
        <template slot-scope="{ row }">
          <span>{{ row.address }}</span>
        </template>
      </el-table-column>
       <el-table-column prop="image" label="营业执照" min-width>
        <!-- 图片的显示 -->
        <template slot-scope="scope">
          <img :src="scope.row.businessLicense" min-width="70" height="70" />
        </template>
      </el-table-column>

        <el-table-column prop="image" label="营业许可证" min-width>
        <!-- 图片的显示 -->
        <template slot-scope="scope">
          <img :src="scope.row.permitForoperation" min-width="70" height="70" />
        </template>
      </el-table-column>
        ...
        <el-table-column prop="image" label="证件照" min-width>
        <!-- 图片的显示 -->
        <template slot-scope="scope">
    ...
    <el-table-column label="申请时间" width="" align="center">
       ...
       <el-table-column label="审核意见" width="" align="center">
        <template slot-scope="{ row }">
          <span>{{ row.handleContent }}</span>
        </template>

      </el-table-column>
        <el-table-column label="审核时间" width="" align="center">
        <template slot-scope="{ row }">
...
<script>
import { findApplyPage,findTypeById } from "@/api/shopsManage";
...
export default {
  ...
  methods: {
    ...
    async findTypeById(id) {
      const name = await findTypeById(id);

      // return ;

      return Promise.resolve(name.shopsRules.shopsType);
    },
    async findAll(data) {
    // const res2 = await findMenuTree();
      this.listLoading = true;
      const res = await findApplyPage(this.listQuery);

      const item = res.shopsApplyList.list;
      this.list = [];
      // 遍历返回的list
      item.map(value => {
        const temp = value;
        this.findTypeById(temp.shopsTypeId).then(name => {
           
             temp.shopsTypeId = name;
           });
        if(temp.handleStatus == 1){
            temp.handleStatus = "待审核";
        }else if(temp.handleStatus == 2){
            temp.handleStatus = "审核通过";
        }else if(temp.handleStatus == 3){
            temp.handleStatus = "审核不通过";
        }
        this.list.push(temp);
      });

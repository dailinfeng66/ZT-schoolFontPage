<template>
  <div class="app-container">
    ...
    >
      ...
      <!-- 一列的结束 -->
      <el-table-column label="订单号" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.orderId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="评论详情" width align="center">
        <template slot-scope="{ row }">
      ...
      <el-table-column label="评论时间 " width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.commentTime }}</span>
        </template>
      </el-table-column>

      <el-table-column label="回复详情 " width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.replyContent }}</span>
        </template>
      </el-table-column>
      <el-table-column label="回复时间 " width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.replyTime }}</span>
        </template>
      </el-table-column>
      <el-table-column label="   回复人  " width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.replyUserId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="评论阅读状态" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.commentStatus }}</span>
        </template>
      </el-table-column> 
      <el-table-column label="回复阅读状态" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.replyStatus }}</span>
        </template>
      </el-table-column>
...
<script>
import { findPage } from "@/api/orderCommentManage";
...
export default {
  ...
  methods: {
    ...
    async findAll(data) {
      ...
      const item = res.orderCommentList.list;
      this.list = [];
      // 遍历返回的list
      item.map(value => {
        const temp = value;

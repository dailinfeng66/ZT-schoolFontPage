  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="申请人" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.applypeople }}</span>
        </template>
      </el-table-column>
      <el-table-column label="申请详情" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.applyContent }}</span>
        </template>
      </el-table-column>
      <el-table-column label="图片id" width="200px" align="center">
        <template slot-scope="{ row }">
          <span>{{ row.image }}</span>
        </template>
      </el-table-column>

      <el-table-column label="售后处理人 " width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.handlePeople }}</span>
        </template>
      </el-table-column>
      <el-table-column label="售后处理详情 " align="center">
        <template slot-scope="{ row }">
          <span>{{ row.handleContent }}</span>
        </template>
      </el-table-column>
      <el-table-column label="售后处理状态 " align="center">
        <template slot-scope="{ row }">
...
<script>
import { findOrderInfo, findShopById } from "@/api/orderSaleManage";
...
export default {
  ...
  methods: {
    ...
    async findAll(data) {
      ...
      const res = await findOrderInfo(this.listQuery);
      console.log(res);
      //判断是否返回正确结果
      if (res.code == 200) {
        const item = res.afterSaleList.list;
        this.list = [];
        // 遍历返回的list
        item.map(value => {
          const temp = value;

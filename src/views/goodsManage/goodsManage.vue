<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="商品描述" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.goodsDescribe }}</span>
        </template>
      </el-table-column>
      ...
      <!-- <el-table-column label="darenId" width="200px" align="center">
        <template slot-scope="{ row }">
          <span>{{ row.darenId }}</span>
        </template>
      </el-table-column> -->
      <!-- <el-table-column label="商铺" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.shopsId }}</span>
        </template>
      </el-table-column> -->
      <!-- <el-table-column label="券码规则id" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.couponConfigId }}</span>
        </template>
      </el-table-column> -->
...
<script>
import { findPage, insertGoods } from "@/api/goodsManage";
...
export default {
  ...
  methods: {
    ...
    async findAll(data) {
      ...
      const res = await findPage(data);

      this.list=[];
      const item = res.goodsList.list;
      item.map(value => {
        const temp = value;
        if(temp.type == 1){
            temp.type = "文创";
      ...
      this.total = res.goodsList.total;

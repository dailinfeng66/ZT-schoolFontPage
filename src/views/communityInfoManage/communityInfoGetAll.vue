  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="描述" width="110px" align="center">
        <template slot-scope="{ row }">
          <span>{{ row.communityDescribe }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      ...
      >
        ...
        <el-form-item label="社区地址">
          <el-input v-model="temp.communityDescribe" />
        </el-form-item>
        <!-- 需要添加字段的话可以直接加 -->
      </el-form>
...
export default {
  ...
  methods: {
    ...
    async getCommunity() {
      ...
      const res = await getCommunityInfo();
      console.log(res);
      const item = res.CommunityList.list;
      this.list = [];
      // 遍历返回的list
      item.map(value => {
        const temp = value;
    ...
    createData() {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          ...
          createCommunity(tempData).then(res => {

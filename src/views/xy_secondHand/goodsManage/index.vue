...
<div class="app-container">
    <div class="filter-container">
        <!-- 标题 input框 -->
        <el-input v-model="query" placeholder="用户ID" style="width: 200px;" class="filter-item" @keyup.enter.native="getM" />
        <!-- 搜索按钮 -->
        ...
        <!-- 二级分类 -->
        <el-select @change="selectCatalog2(catalogValue2)" v-model="catalogValue2" placeholder="请选择">
            <el-option v-for="item in catalog2" :key="item.catalogId" :label="item.name" :value="item.catalogId">
            </el-option>
        </el-select>
    </div>
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;" @sort-change="sortChange">
        ...
        <el-table-column prop="image" label="封面图片" align="center" width="200px">
            <!-- 图片的显示 -->
        ...
        <el-table-column label="一级分类" width align="center">
            <template slot-scope="{ row }">
                <span>{{ row.catalog1 }}</span>
            </template>
        </el-table-column>
        <el-table-column label="二级分类" width align="center">
            <template slot-scope="{ row }">
        ...
        <el-table-column label="商品发布时间" width align="center">
            <template slot-scope="{ row }">
                <span>{{ row.goodsReleaseTime }}</span>
            </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" width="200px" class-name="small-padding fixed-width">
            ...
            <template slot-scope="{ row }">
                ...
                <el-button type="danger" size="small" @click="noPass(row)">不通过</el-button>
            </template>
        </el-table-column>
    </el-table>
    <!-- 下面是修改操作的东西 -->
...
export default {
    ...
    methods: {
        ...
        async selectCatalog(val) {
            ...
            this.list = res1.queryResult.list;
            this.total = res1.queryResult.total;
            this.catalog2 = res.queryResult.list; //设置二级下拉框的内容  
            this.listLoading = false;
        },

...
<script>
import { findAll, findShopName } from "@/api/MerchantAppliManagement";
...
export default {
  ...
  methods: {
    ...
    async getAll(data) {
      ...
      const item = res.merchantApplyList.list;
      console.log(item);
      this.list = [];
      // 遍历返回的list
      item.map(async value => {
        const temp = value;
        if (value.handleStatus == 1) {
          temp.handleStatus = "待审核";
        } else if (value.handleStatus == 2) {
      ...
      this.total = res.merchantApplyList.total;

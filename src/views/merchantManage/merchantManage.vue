<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="商铺id" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.shopsId }}</span>
        </template>
      </el-table-column>

      <el-table-column label="身份证明" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.idCard }}</span>
        </template>
      </el-table-column>
      <el-table-column label="证件照" width align="center">
        <template slot-scope="{ row }">
          <span>{{ row.photo }}</span>
        </template>
      </el-table-column>
      <el-table-column label="是否主商户" width="200px" align="center">
        <template slot-scope="{ row }">
          <span>{{ row.isMerchant }}</span>
        </template>
      </el-table-column>
...
<script>
import { findPage } from "@/api/merchantManage";
...
export default {
  ...
  methods: {
    ...
    // const res2 = await findMenuTree();
      ...
      const item = res.merchantList.list;
      this.list = [];
      // 遍历返回的list
      item.map(value => {
        const temp = value;
        if(temp.isMerchant == 1){
            temp.isMerchant = "是";
        }else if(temp.isMerchant == 0){
            temp.isMerchant = "不是";
        }
        this.list.push(temp);
      });
